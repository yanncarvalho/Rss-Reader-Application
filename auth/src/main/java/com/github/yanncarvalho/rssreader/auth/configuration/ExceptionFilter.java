...
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.ACESS_FORBIDDEN;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.DATA_NULL;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.GENERIC_ERROR;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.INCORRET_CREDENTIALS;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.INTERNAL_SERVER_ERROR;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.INVALID_REQUEST;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.JWT_BAD;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.JWT_ENCODE_PROBLEM;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.LOGGER_EXCEPTION_DEFAULT;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.NOT_FOUND;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.NOT_FOUND_URL;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.USERNAME_NOT_UNIQUE;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.USER_NOT_FOUND;
import static com.github.yanncarvalho.rssreader.auth.configuration.DefaultValue.USER_SINGLE_ADMIN;
...
public class ExceptionFilter extends Http403ForbiddenEntryPoint {
	...
	public ErrorRes processException(Exception ex, HttpServletRequest request) {
		...
		return new ErrorRes(INTERNAL_SERVER_ERROR, ex.getLocalizedMessage());
	}
	...
	public ErrorRes processException(HttpMessageNotReadableException ex, HttpServletRequest request) {
		...
		return new ErrorRes(GENERIC_ERROR,  ex.getLocalizedMessage().replaceAll("\\s\\(.*\\)", "."));
	}
	...
	public ErrorRes processUsernameNotFoundException(UsernameNotFoundException ex, HttpServletRequest request) {
		logger.error(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex));
		return new ErrorRes(USER_NOT_FOUND, ex.getLocalizedMessage());
	}
	...
	public ErrorRes processSingleUserAdminException(UserSingleAdminException ex, HttpServletRequest request) {
		logger.error(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex));
    	return new ErrorRes(USER_SINGLE_ADMIN, ex.getLocalizedMessage());
	}
	...
	public ErrorRes processUsernameNotUniqueException(UsernameNotUniqueException ex, HttpServletRequest request) {
		logger.warn(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex.getLocalizedMessage()));
		return new ErrorRes(USERNAME_NOT_UNIQUE, ex.getLocalizedMessage());
	}
	...
	public ErrorRes processBadJwtException(BadJwtException ex, HttpServletRequest request) {
		logger.warn(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex.getLocalizedMessage()));
		return new ErrorRes(JWT_BAD, ex.getLocalizedMessage());
	}
	...
	public ErrorRes procesJwtEncodingException(JwtEncodingException ex, HttpServletRequest request) {
		logger.warn(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex.getLocalizedMessage()));
		return new ErrorRes(JWT_ENCODE_PROBLEM, ex.getLocalizedMessage());
	}
	...
	public ErrorRes processBadCredentialsException(BadCredentialsException ex, HttpServletRequest request) {
		logger.warn(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex.getLocalizedMessage()));
		return new ErrorRes(INCORRET_CREDENTIALS, ex.getLocalizedMessage());
	}
	...
	public ErrorRes procesNullPointerException(NullPointerException ex, HttpServletRequest request) {
		logger.warn(LOGGER_EXCEPTION_DEFAULT(request.getRemoteAddr(), ex.getLocalizedMessage()));
		return new ErrorRes(DATA_NULL, ex.getLocalizedMessage());
	}
	...
	public void commence(HttpServletRequest request, HttpServletResponse res, 
		...
		res.getWriter().write(new ObjectMapper()
				.writeValueAsString(new ErrorRes(ACESS_FORBIDDEN, authException.getLocalizedMessage())));
	}
